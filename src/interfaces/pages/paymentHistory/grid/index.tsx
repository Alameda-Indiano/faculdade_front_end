...
export const PaymentHistoryGrid = () => {
	...
	const { filters, onFilters } = useFilters<IPaymentHistoryFilters>({
		initialFilters: {
			search: '',
		},
	...
	const queryDeletePaymentHistory = async (
		...
		await itemsSelectedDeletion.reduce(async (previousPromise, id) => {
			...
			try {
				const data = await PaymentHistoryRepository.delete(id);
				results.push(data);
			} catch (error) {
				console.error(error);
	...
	const handleDeleteRow = useCallback(
		async (id: string) => {
			await queryDeletePaymentHistory([id]);
	...
	const handleEditRow = useCallback(
		(id: string) => {
			router.push(Routes.paymentHistoryEdit(id));
		},
		[router],
	);
