...
export const mockBodyMeasurementRepository: IBodyMeasurementRepository = {
  create: async (data: IBodyMeasurementEntity) => {
    ...
    const newMeasurement: IBodyMeasurementEntity = { ...data, id, created_at: timestamp, updated_at: timestamp };
  ...
  getById: async (bodyMeasurementId: string) => {
    await simulateDelay(500);
    const found = mockBodyMeasurements.find(m => m.id === bodyMeasurementId);
    if (!found) {
  ...
  update: async (data: IBodyMeasurementEntity) => {
    ...
    const updatedMeasurement = { ...mockBodyMeasurements[index], ...data, updated_at: new Date().toISOString() };
  ...
  delete: async (bodyMeasurementId: string) => {
    await simulateDelay(500);
    const index = mockBodyMeasurements.findIndex(m => m.id === bodyMeasurementId);
    if (index === -1) {
      return { success: false, message: 'Medição não encontrada.' };
    }
